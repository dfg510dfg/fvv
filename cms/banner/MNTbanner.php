...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>MANTENIMIENTO DE BANNERS</title>
            ...
            function irA(accion){
                if(accion=='INS'){
                    ...
                    if(!activo){
                        ...
                        window.location='UPDbanner.php?id='+id;
                    } 
                }else if(accion=='DEL'){
                    var frm = document.forms[0];
                    var f = frm.length;
                    ...
                    if(!activo){
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'ControladorBanner.php',
                            data: 'accion=DEL_FIS&txhId=' + id,
                            success:function(msj){
                                if ( msj == 1 ){
                                    alert('El registro fue eliminado satisfactoriamente');
                                    window.location='MNTbanner.php';
                                }
                                else{
                                    alert('Error');
                                }
    ...
    <body>
        ...
        <div style="width: auto; border-radius:10px; border:solid 5px #000;background-image:url(../img/bgpanel.jpg);">
            ...
            <div style="width: 900px;height:40px;background-color:grey; text-align: left;color:#fff;padding-left: 5px;">
                <h1 style="padding: 0; margin: 0;">Mantenimiento de Banner</h1>
            </div>
            <br/>    
            ...
            <form id="demo-form" ata-validate="parsley">
                            ...
                            <td class="label">Buscar por : </td>
                                    <td>
                                        <table border="0" cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td><input type="radio" name="rdbBuscar" id="rdbTipoLista1" onclick="validar();" value="nombre" checked <?=($_REQUEST["rdbBuscar"]=="nombre")?"checked":""?>/> Nombre Banner</td>
                                                <td width="50"></td>
                ...
                print($objMysql->getGrillaHTMLTablet(
                        $objDatos->getConsulta(array("listarBanner","",(!$_GET["campo"])?"codigo":$_GET["campo"],(!$_GET["ordenar"])?"asc":$_GET["ordenar"],(($_POST["rdbBuscar"])?$_POST["rdbBuscar"]:$_GET["rdbBuscar"]),(($_POST["txtValorabuscar"])?$_POST["txtValorabuscar"]:$_GET["txtValorabuscar"]),$page,$mostrarRegistros)),
                        "850", 
                        array("Código", "Nombre", "Foto", "Descripcion", "Orden", "Cargar Pagina", "Estado"), 
                        array(75, 120, 120, 220, 80,120,80), 
                        array($_GET["campo"], $_GET["ordenar"], $page, $objDatos->getConsulta(array("listarBannerTot","",(!$_GET["campo"])?"codigo":$_GET["campo"],(!$_GET["ordenar"])?"asc":$_GET["ordenar"],(($_POST["rdbBuscar"])?$_POST["rdbBuscar"]:$_GET["rdbBuscar"]),(($_POST["txtValorabuscar"])?$_POST["txtValorabuscar"]:$_GET["txtValorabuscar"]),"")), (($_POST["txtValorabuscar"])?$_POST["txtValorabuscar"]:$_GET["txtValorabuscar"]), (($_POST["rdbBuscar"])?$_POST["rdbBuscar"]:$_GET["rdbBuscar"]),$mostrarRegistros)
                    ));
