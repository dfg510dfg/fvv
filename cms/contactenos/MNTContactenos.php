...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>MANTENIMIENTO DE CONSULTAS</title>
        ...
        <script>
            ...
            function irA(accion){
                if(accion=='UPD'){
                    ...
                    if(!activo){
                        ...
                        window.location='UPDContactenos.php?id='+id;
                    } 
                }else if(accion=='DEL'){
                    var frm = document.forms[0];
                    var f = frm.length;
            ...
            function buscar(objform){
                ...
                if(bol){
                    objform.method="post";
                    objform.action="MNTContactenos.php";
                    objform.submit();
                }else {
                    return 0;
                }
    ...
    <body>
        ...
        <div style="width: auto; border-radius:10px; border:solid 5px #000;background-image:url(../img/bgpanel.jpg);">
            ...
            <div style="width: 900px;height:40px;background-color:grey; text-align: left;color:#fff;padding-left: 5px;">
                <h1 style="padding: 0; margin: 0;">Mantenimiento de Consultas</h1>
            </div>
            <br/>    
            ...
            <form id="demo-form" ata-validate="parsley">
                            ...
                            <td class="label">Buscar por : </td>
                                    <td>
                                        <table border="0" cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td><input type="radio" name="rdbBuscar" id="rdbTipoLista1" onclick="validar();" value="nombre" checked <?=($_REQUEST["rdbBuscar"]=="nombre")?"checked":""?>/> Nombre Contacto</td>
                                                <td width="50"></td>
                ...
                $objDatos = new Datos();
                $mostrarRegistros = 8;
                print($objMysql->getGrillaHTMLContactenos(
                        $objDatos->getConsulta(array("listarContactenos","",(!$_GET["campo"])?"codigo":$_GET["campo"],(!$_GET["ordenar"])?"asc":$_GET["ordenar"],(($_POST["rdbBuscar"])?$_POST["rdbBuscar"]:$_GET["rdbBuscar"]),(($_POST["txtValorabuscar"])?$_POST["txtValorabuscar"]:$_GET["txtValorabuscar"]),$page,$mostrarRegistros)),
                        "850", 
                        array("Código", "Fecha", "Hora", "Contacto", "Telefono","Correo","Estado"), 
                        array(80, 100,120, 150, 110,130,90), 
                        array($_GET["campo"], $_GET["ordenar"], $page, $objDatos->getConsulta(array("listarContactenosTot","",(!$_GET["campo"])?"codigo":$_GET["campo"],(!$_GET["ordenar"])?"asc":$_GET["ordenar"],(($_POST["rdbBuscar"])?$_POST["rdbBuscar"]:$_GET["rdbBuscar"]),(($_POST["txtValorabuscar"])?$_POST["txtValorabuscar"]:$_GET["txtValorabuscar"]),"")), (($_POST["txtValorabuscar"])?$_POST["txtValorabuscar"]:$_GET["txtValorabuscar"]), (($_POST["rdbBuscar"])?$_POST["rdbBuscar"]:$_GET["rdbBuscar"]),$mostrarRegistros,3)
                    ));
