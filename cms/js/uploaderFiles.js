...
{       
	...
	var uploaded_files_location = "../file_uploaded/";
	new AjaxUpload($('#vpb_upload_button'), 
	{
		action: '../tecnologia/uploader.php',
		name: 'file_to_upload',
		onSubmit: function(file, file_extensions)
		{
			...
			if (!(file_extensions && /^(jpg|png|jpeg|gif|pdf|docx|doc|rtf|txt|rar|mp3|xls)$/.test(file_extensions)))
			{
		...
		onComplete: function(file, response)
		{
			if(response === "file_uploaded_successfully")
			{
				...
				else if(type_of_file_uploaded == "rar" || type_of_file_uploaded == "RAR")
				{
					$('#vpb_uploads_displayer').append("<div class='vpb_image_wrappers' id='fileID"+vpb_file_names+"' style='padding-top:60px;padding-bottom:70px;' align='center'><span class='ccc'><a href='"+uploaded_files_location+"/"+file+"' target='_blank'>Archivo Comprimido<br><br>Click here to view it</a></span><div id='vpb_remove_button"+vpb_file_names+"' class='vpb_remove_button' onclick='remove_unwanted_file(\""+vpb_file_names+"\",\""+file+"\");'>Remove</div></div>");
				}
				else if(type_of_file_uploaded == "mp3" || type_of_file_uploaded == "MP3")
				{
					$('#vpb_uploads_displayer').append("<div class='vpb_image_wrappers' id='fileID"+vpb_file_names+"' style='padding-top:60px;padding-bottom:70px;' align='center'><span class='ccc'><a href='"+uploaded_files_location+"/"+file+"' target='_blank'>Archivo de audio<br><br>Click here to view it</a></span><div id='vpb_remove_button"+vpb_file_names+"' class='vpb_remove_button' onclick='remove_unwanted_file(\""+vpb_file_names+"\",\""+file+"\");'>Remove</div></div>");
				}
				else if(type_of_file_uploaded == "xls" || type_of_file_uploaded == "XLS")
				{
					$('#vpb_uploads_displayer').append("<div class='vpb_image_wrappers' id='fileID"+vpb_file_names+"' style='padding-top:60px;padding-bottom:70px;' align='center'><span class='ccc'><a href='"+uploaded_files_location+"/"+file+"' target='_blank'>Archivo Excel<br><br>Click here to view it</a></span><div id='vpb_remove_button"+vpb_file_names+"' class='vpb_remove_button' onclick='remove_unwanted_file(\""+vpb_file_names+"\",\""+file+"\");'>Remove</div></div>");
				}
				else
				{
       ...
       //  remove_unwanted_file('3','3.jpg');
{
	...
	if(confirm("Si está seguro que desea eliminar el archivo: "+file+" de click en OK o de otro modo, Cancelar."))
	{
                ...
                $.ajax({
			type: "POST",
			url: "../tecnologia/remove_unwanted_files.php",
			data: dataString,
			cache: false,
			beforeSend: function() 
			{
